...
export default async function reportEmail(props) {
    try {
        console.log('go to send report in email');
        let result=await queryDb();


        const data =await  createStagesResult();
        return { message: 'report email ok', data };
    }
    catch (error) {
        logger.error('fail to send report email: ' + error.message);
        throw { message: 'fail to send repot email', status: error.status || 501, data: { status: error.status || 501 } };
    }
...
const queryDb=async()=>{
    try{
        ...
        let query={select:['ID','USERNAME',{raw:`get_part_dscr (   21, 5,342,1,  API_USERS.ACTIVE,   'X',  123,null ) as  get_part_dscr`}],from:'API_USERS'};
        let result=await client.request('db',query);
        return result
    }
    catch(error){
    console.log('fail to query db:',error.message);
    }
